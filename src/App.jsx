    ...
    >
      ...
      <div
        className={`flex flex-col space-y-6 w-[600px] md:w-[100%] z-10 p-4 ${
          darkTheme ? "text-black" : "text-black"
        }`}
      >
        <div className=" w-full flex items-center justify-between">
          <h1 className=" uppercase text-2xl font-medium text-black tracking-widest mb-4 md:text-3xl">
            {/* Task Manager */}
            My Tasks
          </h1>
          ...
          {darkTheme ? (
            <MdSunny
              onClick={toggleTheme}
              className={`bg-gray-300 w-10 h-10 cursor-pointer dark:bg-gray-700 p-2 rounded-3xl  bottom-5 right-5 ${
                darkTheme ? "text-yellow-400" : "text-black"
              }`}
            />
          ) : (
            <MdDarkMode
              onClick={toggleTheme}
              className={`bg-gray-300 w-10 h-10 cursor-pointer dark:bg-gray-700 p-2 rounded-3xl  bottom-5 right-5 ${
                darkTheme ? "text-white" : "text-blue-400"
              }`}
            />
          )}
        </div>
